...
class Travels extends CI_Controller {
	...
	public function login(){
		...
		if($this->form_validation->run() === FALSE)		{
			...
			if($this->travel_model->login($post)){
				redirect('travels');
			}
	...
	public function register(){
		...
		if($this->form_validation->run() === FALSE)		{
			...
			$post = $this->input->post();
			if($this->travel_model->register($post) && $this->travel_model->login($post)){
				redirect('travels');
			}
			redirect('unanticipated_error');
		}
	...
	public function check_preexisting_username($post_username){
		$record = $this->travel_model->show_by_username($post_username);
		if($record){
	...
	public function travels_page(){
		...
		$trips = $this->travel_model->get_all_trips($active_id);
		$this->load->view('travels_view',['trips'=> $trips]);
	}
	public function destination_page($id){
		$destination['details'] = $this->travel_model->show_destination_details($id);
		$destination['attendees'] = $this->travel_model->show_all_going_to_destination($id);
		$this->load->view('destination_view',['destination'=> $destination]);
	}
	public function join_trip($id){
		$this->travel_model->join_trip($id);
		redirect('travels');
	}
	...
	public function add_destination(){
		...
		if($this->form_validation->run() === FALSE)		{
			...
			$this->travel_model->add_destination($post);
			redirect('/travels');
		}
	}
